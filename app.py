...
# ── Tabs ─────────────────────────────────────────────────────
t1, t2, t3 = st.tabs(["✏️ แจ้งปัญหา", "🔍 ติดตามสถานะ", "🛡 Admin"])
...
with t1:
    ...
    with col:
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:1.25rem;">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1A6BE0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
          <span style="font-size:1.25rem;font-weight:700;color:#1a1a2e;">แจ้งปัญหา</span>
        ...
        category = st.selectbox("หมวดปัญหา", CATS)
        location = st.text_input("สถานที่ *", placeholder="เช่น อาคาร A ชั้น 3, ทางเข้าอาคาร")
        detail   = st.text_area("รายละเอียดปัญหา (ไม่บังคับ)", height=120,
                                 placeholder="อธิบายปัญหาที่พบ...")
        image    = st.file_uploader("อัปโหลดรูปภาพ (ไม่บังคับ)", type=["jpg","jpeg","png","gif"])
        confirmed = st.checkbox("ยืนยันการแจ้งปัญหา")

        submit = st.button("📤 ส่งรายงาน", type="primary")

        if submit:
            errs = []
            if not name.strip():     errs.append("ชื่อผู้แจ้ง")
...
with t2:
    ...
    with col:
        st.markdown("""
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:1.25rem;">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1A6BE0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
          <span style="font-size:1.25rem;font-weight:700;color:#1a1a2e;">ติดตามสถานะ</span>
        </div>""", unsafe_allow_html=True)

        st.markdown("**รหัสการแจ้ง**")
        cx, cy = st.columns([5, 1], vertical_alignment="bottom")
        with cx:
            tid = st.text_input("รหัสการแจ้ง", placeholder="RP-XXXXXX",
                                 label_visibility="collapsed")
        with cy:
            go = st.button("ตรวจสอบ", type="primary", use_container_width=True)
        ...
        if go:
            ...
            if found:
                ...
                if found.get("ImageUrl"):
                    # Show image section like in screenshot
                    img_name = found.get("ImageName", "รูปภาพ")
                    st.markdown(f"""
                    <div class="dcard" style="margin-top:12px;">
                      <div class="dcard-hdr" style="cursor:pointer;">
                        <span>รูปภาพที่แนบ — {img_name}</span>
                        <span>∧</span>
                      </div>
                    </div>""", unsafe_allow_html=True)
                    st.image(found["ImageUrl"], caption=img_name, width=340)
                else:
                    st.markdown(f"""
                    <div class="dcard" style="margin-top:12px;">
                      <div class="dcard-hdr">
                        <span>รูปภาพที่แนบ</span>
                      </div>
                      <div style="padding:2.5rem;text-align:center;color:#9aa0a6;">
                        <div style="font-size:2rem;margin-bottom:8px;">🖼</div>
                        <div style="font-size:13px;">ไม่มีรูปภาพที่แนบ</div>
                      </div>
                    </div>""", unsafe_allow_html=True)
            else:
...
with t3:
    ...
    with col:
        ...
        if not st.session_state.admin_logged_in:
            st.markdown("""
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:1.25rem;">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1A6BE0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>
              <span style="font-size:1.25rem;font-weight:700;color:#1a1a2e;">Admin Login</span>
            </div>""", unsafe_allow_html=True)
            ...
            pwd = st.text_input("รหัสผ่าน", type="password",
                                 ...
                                 label_visibility="collapsed")
            login_btn = st.button("เข้าสู่ระบบ Admin", type="primary", use_container_width=False)
        ...
        with hc1:
            ...
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:1rem;">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1A6BE0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="8" y1="21" x2="16" y2="21"/><line x1="12" y1="17" x2="12" y2="21"/></svg>
              <span style="font-size:1.25rem;font-weight:700;color:#1a1a2e;">ภาพรวม</span>
            </div>""", unsafe_allow_html=True)
        with hc2:
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:0.75rem;">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1A6BE0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>
          <span style="font-size:1.25rem;font-weight:700;color:#1a1a2e;">รายการแจ้ง</span>
        </div>""", unsafe_allow_html=True)
        st.markdown('<div class="scard" style="padding:16px 20px;margin-bottom:16px;">',
                    unsafe_allow_html=True)
        f1, f2, f3 = st.columns([3, 2, 2])
        with f1:
            st.markdown("**ค้นหา ID**")
            srch = st.text_input("ค้นหา ID", placeholder="RP-XXXXXX", label_visibility="collapsed")
        with f2:
            st.markdown("**เดือน**")
            mon  = st.selectbox("เดือน", ["ทั้งหมด"] + df["Month"].unique().tolist(), label_visibility="collapsed")
        with f3:
            st.markdown("**สถานะ**")
            sf   = st.selectbox("สถานะ", ["ทั้งหมด"]+STATUSES, label_visibility="collapsed")
        ...
        st.markdown("""
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:0.75rem;">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1A6BE0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>
          <span style="font-size:1.25rem;font-weight:700;color:#1a1a2e;">เปลี่ยนสถานะ</span>
        </div>""", unsafe_allow_html=True)
        st.markdown('<div class="scard">', unsafe_allow_html=True)
        u1, u2, u3 = st.columns([2, 2, 1], vertical_alignment="bottom")
        with u1:
            st.markdown("**เลือก ID**")
            sel_id = st.selectbox("ID", df["ID"], label_visibility="collapsed")
        with u2:
            st.markdown("**สถานะใหม่**")
        ...
        st.markdown("""
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:0.75rem;">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1A6BE0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
          <span style="font-size:1.25rem;font-weight:700;color:#1a1a2e;">จัดการรายงาน</span>
        </div>""", unsafe_allow_html=True)
        st.markdown('<div class="scard">', unsafe_allow_html=True)
        st.markdown("**เลือก ID**")
        ...
        with bv: show = st.button("👁 แสดงรายละเอียด", use_container_width=True)
        with be:
            if st.button("✏️ แก้ไขรายการ", use_container_width=True):
                st.session_state.edit_id = vid
                st.session_state.confirm_delete_id = None
        with bd:
            if st.button("🗑 ลบรายงาน", use_container_width=True, type="primary"):
